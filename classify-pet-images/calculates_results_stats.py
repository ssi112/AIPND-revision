...
def calculates_results_stats(results_dic):
    ...
    for key, value in results_dic.items():
        #  accummulate counts
        results_stats_dic['n_images'] += 1  # Z
        if results_dic[key][3] == 1:    #B
            results_stats_dic['n_dogs_img'] += 1
        if results_dic[key][3] == 0:    # D
            results_stats_dic['n_notdogs_img'] += 1
        if (results_dic[key][3] == 1 and results_dic[key][4] == 1): # A
            results_stats_dic['n_match'] += 1
        if results_dic[key][2] == 1:    # Y
            results_stats_dic['n_correct_dogs'] += 1
        if (results_dic[key][3] == 0 and results_dic[key][4] == 0): # C
